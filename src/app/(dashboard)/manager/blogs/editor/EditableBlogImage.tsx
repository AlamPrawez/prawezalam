...
export const EditableBlogImage: React.FC<{
    ...
    // Fixed-size, clipped container for the actual <img> — this is what was
    // missing. Without it, an unstyled <img> renders at its natural
    // intrinsic size and (since the OUTER wrapper is intentionally
    // overflow-visible, to let the settings dropdown pop out) bleeds over
    // whatever sits next to it in the grid, e.g. the body text column.
    // `className` (if passed) is applied on top of these defaults so callers
    // can still override sizing per-layout.
    const renderImageContent = () => (
        <div className="relative w-full h-64 sm:h-72 overflow-hidden rounded-xl border border-slate-800 bg-slate-900">
            <img
    ...
    return (
        <div ref={menuRef} className="relative group/image w-full overflow-visible">
            ...
            {/* Optional alt-text caption, matching the read-only ContentViews render */}
            {src && alt && (
                <p className="p-2 text-xs text-center text-slate-400 italic bg-slate-900 border border-t-0 border-slate-800 rounded-b-xl -mt-1">
                    {alt}
                </p>
            )}
