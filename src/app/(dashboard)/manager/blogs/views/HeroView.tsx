  ...
  return (
    <div className="relative group/text-editor inline-block w-full">
      ...
      <Tag
        ...
        className={`outline-none rounded-md transition focus:bg-indigo-500/10 focus:ring-2 focus:ring-indigo-500/60 empty:before:content-[attr(data-placeholder)] empty:before:text-slate-400 ${className}`}
      />
    </div>
  );
};
...
export const EditableImage: React.FC<{
  ...
  const renderImageContent = () => <img src={src} alt={alt} className={className} />;
  ...
  return (
    <div ref={menuRef} className="relative group/image inline-block w-full">
      {src ? (
        ...
        <div className="w-full h-56 bg-slate-100 dark:bg-slate-900/60 border-2 border-dashed border-slate-300 dark:border-slate-800 rounded-2xl flex flex-col items-center justify-center text-slate-500 dark:text-slate-400 text-xs p-6 space-y-3">
  ...
  return (
    <div className={wrapClass} ref={menuRef}>
      {buttons.map((btn, i) => (
        <div key={btn.id} className="group/item relative inline-flex items-center">
          <a
            ...
            className={`px-6 py-3 rounded-xl text-xs font-bold inline-flex items-center justify-center transition ${
      ...
      <button
        type="button"
        onClick={add}
        className="px-3 py-2 rounded-xl text-xs font-bold border border-dashed border-slate-400 dark:border-slate-700 text-slate-500 hover:text-slate-800 dark:hover:text-slate-300 transition"
        data-editor-only
      >
        + Button
      </button>
...
const EditableBullets: React.FC<{
  ...
  return (
    <>
      {bullets.map((bp, i) => (
        <span key={i} className="group/item relative inline-flex items-center gap-1">
          ...
          <button
            type="button"
            onClick={() => remove(i)}
            className="w-4 h-4 rounded-full bg-red-500/20 text-red-500 hover:bg-red-500/40 text-[10px] flex items-center justify-center opacity-0 group-hover/item:opacity-100 transition"
            data-editor-only
          >
            ×
          </button>
      ...
      <button type="button" onClick={add} className="text-[11px] text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 underline transition" data-editor-only>
        + Add highlight
      </button>
    </>
  );
...
export const ProfileServicesHero: React.FC<{
  ...
  const isLight = sec.bgTheme === 'light';

  return (
    <section className={`relative min-h-[560px] flex items-center py-16 px-6 md:px-16 border-b ${
      isLight ? 'bg-slate-50 text-slate-900 border-slate-200' : 'bg-[#0A0D14] text-white border-slate-800'
    }`}>
      <div className="absolute inset-0 z-0 overflow-hidden">
        <EditableImage
          ...
          className={`w-full h-full object-cover object-right filter grayscale ${isLight ? 'opacity-15' : 'opacity-25'}`}
          onUpdate={(patch) => onChange(patch)}
        />
        <div className={`absolute inset-0 pointer-events-none bg-gradient-to-r ${
          isLight ? 'from-slate-50 via-slate-50/90 to-transparent' : 'from-[#0A0D14] via-[#0A0D14]/90 to-transparent'
        }`} />
      </div>

      <div className="relative z-10 max-w-3xl space-y-6">
        {badgeText !== undefined ? (
          <div className={`group/badge relative inline-flex items-center gap-2 px-3 py-1 rounded-full text-[11px] font-bold tracking-wider uppercase pr-8 ${
            isLight ? 'bg-indigo-50 border border-indigo-200 text-indigo-700' : 'bg-indigo-950/60 border border-indigo-500/30 text-indigo-300'
          }`}>
            ...
            <button
              ...
              className="absolute right-1.5 top-1/2 -translate-y-1/2 p-0.5 rounded-full bg-slate-200 dark:bg-slate-900/80 text-slate-500 hover:text-red-500 opacity-0 group-hover/badge:opacity-100 transition"
              title="Remove badge"
              data-editor-only
            >
          ...
          <div className="pt-1" data-editor-only>
            <button
              ...
              className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[11px] font-bold tracking-wider uppercase border border-dashed transition ${
                isLight ? 'border-slate-300 text-slate-500 hover:text-indigo-600 hover:border-indigo-400' : 'border-slate-700 text-slate-400 hover:text-indigo-300 hover:border-indigo-500/50'
              }`}
            >
        ...
        <EditableText
          ...
          className={`block text-3xl sm:text-4xl md:text-5xl font-black tracking-tight leading-[1.15] ${isLight ? 'text-slate-900' : 'text-white'}`}
        />
        ...
        <EditableText
          ...
          className={`block text-xs sm:text-sm md:text-base leading-relaxed whitespace-pre-line font-normal ${isLight ? 'text-slate-600' : 'text-slate-300/90'}`}
        />

        <div className="pt-2">
          <EditableButtons
            buttons={buttons}
            onChange={(next) => onChange({ buttons: next })}
            primaryClass={isLight ? 'bg-indigo-600 text-white hover:bg-indigo-500 shadow-md' : 'bg-white text-slate-950 hover:bg-slate-200 shadow-md'}
            secondaryClass={isLight ? 'bg-white text-slate-700 border border-slate-300 hover:bg-slate-100' : 'bg-slate-900/90 text-slate-200 border border-slate-700/80 hover:bg-slate-800'}
          />
        </div>
      </div>
    </section>
...
const StandardHero: React.FC<{ sec: PageSectionItem; onChange: OnChange }> = ({ sec, onChange }) => {
  ...
  const themeClasses = {
    ...
    indigo: 'bg-indigo-950 text-white',
    slate: 'bg-slate-900 text-white',
    dark: 'bg-slate-950 text-white',
  }[sec.bgTheme || (isLight ? 'light' : 'dark')];

  const imageBlock = (
    <div className={`rounded-2xl shadow-2xl relative overflow-hidden ${isLight ? 'border border-slate-200 shadow-slate-200/50' : 'border border-slate-800'}`}>
      <EditableImage
        src={sec.imageUrl}
        alt={sec.imageAlt || sec.title}
        imageLinkUrl={sec.imageLinkUrl}
  ...
  return (
    <section className={`relative py-16 px-6 ${themeClasses}`}>
      <div className="max-w-6xl mx-auto relative z-10 grid grid-cols-1 md:grid-cols-12 gap-8 items-center">
        ...
        >
          <EditableText
            ...
            onCommit={(v) => onChange({ title: v })}
            placeholder="Hero title…"
            className={`block text-4xl font-extrabold tracking-tight mb-4 ${isLight ? 'text-slate-900' : 'text-white'}`}
          />
          <EditableText
            as="p"
            value={sec.subtitle}
            onCommit={(v) => onChange({ subtitle: v })}
            placeholder="Hero subtitle…"
            className={`block text-base leading-relaxed mb-6 ${isLight ? 'text-slate-600' : 'text-slate-300'}`}
          />

          <div className={`space-y-2 mb-6 text-xs ${isLight ? 'text-slate-600' : 'text-slate-300'} ${isCentered ? 'items-center flex flex-col' : ''}`}>
            <EditableBullets
          ...
          <EditableButtons
            ...
            secondaryClass={isLight ? 'bg-slate-100 text-slate-800 border border-slate-300 hover:bg-slate-200' : 'bg-slate-800 text-slate-200 border border-slate-700 hover:bg-slate-700'}
...
export const InteractiveCodeHero: React.FC<{
  ...
  return (
    <section className={`relative py-20 px-6 md:px-16 border-b ${
      isLight ? 'bg-slate-50 text-slate-900 border-slate-200' : 'bg-slate-950 text-white border-slate-800/80'
    }`}>
      ...
      <div className="relative z-10 max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
        <div className="lg:col-span-7 space-y-6">
          {badgeText !== undefined ? (
            <div className={`group/badge relative inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-semibold tracking-wide pr-8 ${
              isLight ? 'bg-emerald-50 border border-emerald-300 text-emerald-700' : 'bg-emerald-950/60 border border-emerald-500/30 text-emerald-400'
            }`}>
            ...
            <div className="pt-1" data-editor-only>
              <button
                ...
                className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold border border-dashed transition ${
                  isLight ? 'border-slate-300 text-slate-500 hover:text-emerald-600 hover:border-emerald-400' : 'border-slate-700 text-slate-400 hover:text-emerald-400 hover:border-emerald-500/50'
                }`}
              >
          ...
          <EditableText
            ...
            onCommit={(v) => onChange({ title: v })}
            placeholder="Hero title…"
            className={`block text-3xl sm:text-4xl md:text-5xl font-extrabold tracking-tight leading-tight ${isLight ? 'text-slate-900' : 'text-white'}`}
          />
          ...
          <EditableText
            ...
            onCommit={(v) => onChange({ subtitle: v })}
            placeholder="Hero subtitle…"
            className={`block text-sm md:text-base leading-relaxed max-w-xl ${isLight ? 'text-slate-600' : 'text-slate-300'}`}
          />
          ...
          <div className="flex flex-wrap gap-2 pt-1">
            <EditableBullets
              ...
              onChange={(next) => onChange({ bulletPoints: next })}
              renderItem={(text) => (
                <span className={`px-3 py-1 rounded-lg border text-xs font-medium ${
          ...
          <div className="pt-3">
            <EditableButtons
              ...
              secondaryClass={isLight ? 'bg-white text-slate-800 border border-slate-300 hover:bg-slate-100' : 'bg-slate-900 text-slate-200 border border-slate-700/80 hover:bg-slate-800'}
            />
          </div>
        </div>

        <div className="lg:col-span-5">
          <div className="rounded-2xl bg-slate-900 border border-slate-800 shadow-2xl overflow-hidden text-white">
            ...
            <div className="p-5 font-mono text-xs text-slate-300 space-y-2 overflow-x-auto leading-relaxed">
              ...
              <p>
                <span className="text-purple-400">from</span> app.services <span className="text-purple-400">import</span> AIStreamer
              </p>
              <br />
              <p>
                <span className="text-blue-400">@app</span>.post(<span className="text-emerald-300">"/v1/build"</span>)
              </p>
              <p>
                <span className="text-purple-400">async def</span> <span className="text-yellow-300">execute_pipeline</span>(payload: ProjectSchema):
              </p>
              <p className="pl-4 text-slate-300">
                result = <span className="text-purple-400">await</span> AIStreamer.deploy(payload)
              </p>
              <p className="pl-4 text-purple-400">
                return <span className="text-slate-300">{`{"status": "success", "score": 100}`}</span>
              </p>
            </div>
