...
export default function SeoSection({
  ...
  const parseAndPopulateSeo = (rawText: string) => {
    ...
    // 0. Extract raw <script type="application/ld+json"> blocks if present
    const scriptMatches = processingText.match(/<script[^>]*type=["']application\/ld\+json["'][^>]*>[\s\S]*?<\/script>/gi);
    if (scriptMatches && scriptMatches.length > 0) {
      const combinedLdJson = scriptMatches.join('\n\n');
      setValue('seo.ldjson' as never, combinedLdJson as never, { shouldValidate: true, shouldDirty: true });
      
      // Remove extracted script block so line-by-line parser isn't polluted by JSON lines
      processingText = processingText.replace(/<script[^>]*type=["']application\/ld\+json["'][^>]*>[\s\S]*?<\/script>/gi, '');
    }
    ...
    lines.forEach((line) => {
      ...
      // 7. LDJSON / Schema Key Parsing (if provided as a key-value pair)
      if (line.match(/^(ldjson|schema|jsonld|json-ld)[\:\=]/i) && !scriptMatches) {
        const ldVal = line.replace(/^(ldjson|schema|jsonld|json-ld)[\:\=]/i, '').trim();
        setValue('seo.ldjson' as never, ldVal as never, { shouldValidate: true, shouldDirty: true });
      }
    });
  };
        ...
        <div className="md:col-span-2 pt-2 border-t border-slate-100">
          <div className="flex justify-between items-center mb-1.5">
            ...
            <span className="text-[10px] text-slate-400 font-medium">
              Accepts raw JSON or &lt;script type="application/ld+json"&gt; tags
            </span>
          </div>
          <textarea
            {...register('seo.ldjson' as never)}
            rows={6}
            placeholder={`<script type="application/ld+json">\n{\n  "@context": "https://schema.org",\n  "@graph": [\n    {\n      "@type": "Service",\n      "serviceType": "Next.js Development"\n    }\n  ]\n}\n</script>`}
            className="w-full p-3.5 text-xs font-mono border border-slate-800 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 bg-slate-900 text-emerald-400 placeholder:text-slate-600 shadow-inner"
          />
        </div>
      </div>
          ...
          <div className="bg-white rounded-2xl max-w-xl w-full p-6 space-y-4 shadow-xl border border-slate-100">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <h3 className="text-base font-bold text-slate-800 flex items-center gap-2">
                ...
                Bulk Auto-Fill SEO & Schema Fields
              </h3>
              <button
                type="button"
